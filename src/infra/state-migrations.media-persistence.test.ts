...
describe("legacy media persistence doctor migration", () => {
  ...
  it("migrates databases whose transcript timestamp aggregate exceeds the safe-integer range", () => {
    const stateDir = makeTempDir(tempDirs, "media-persistence-overflow-");
    const env = { OPENCLAW_STATE_DIR: stateDir };
    // Each created_at is ~10^15; 10 rows sum to ~10^16, well above
    // Number.MAX_SAFE_INTEGER (9.007 × 10^15). The old SQL returned the SUM
    // as a SQLite INTEGER, which DatabaseSync rejects with RangeError before
    // the application-level coercion helper can run.
    const overflowTimestamp = 1_000_000_000_000_000;
    const events: FixtureEvent[] = Array.from({ length: 10 }, (_, i) =>
      createEvent({
        id: `event-${i}`,
        parentId: null,
        timestamp: overflowTimestamp + i,
        message: { role: "user", MediaPath: `/media/a-${i}.png` },
      }),
    );
    const databasePath = createLegacyDatabaseFixture({
      env,
      eventsBySession: { overflow: events },
    });

    const result = migrateLegacyMediaPersistence({ env });
    expect(result.warnings).toHaveLength(0);
    expect(result.changes.join("\n")).toContain("Migrated media persistence");
    const snapshot = readDatabaseSnapshot(databasePath);
