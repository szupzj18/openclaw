...
type MediaSourceVersion = {
  dataVersion: number;
  trajectoryBytes: bigint;
  trajectoryRows: number;
  transcriptBytes: bigint;
  transcriptCreatedAt: bigint;
...
function readMediaSourceVersion(database: DatabaseSync): MediaSourceVersion {
  ...
  const counts = database
    .prepare(
      `SELECT
        ...
        CAST((SELECT COALESCE(SUM(LENGTH(event_json)), 0) FROM transcript_events) AS TEXT) AS transcript_bytes,
        CAST((SELECT COALESCE(SUM(created_at), 0) FROM transcript_events) AS TEXT) AS transcript_created_at,
        (SELECT COUNT(*) FROM trajectory_runtime_events) AS trajectory_rows,
        CAST((SELECT COALESCE(SUM(LENGTH(event_json)), 0) FROM trajectory_runtime_events) AS TEXT) AS trajectory_bytes`,
    )
    .get();
  const number = (value: unknown): number =>
    typeof value === "bigint" ? Number(value) : typeof value === "number" ? value : 0;
  // SUM aggregates can exceed Number.MAX_SAFE_INTEGER (e.g. SUM(created_at)
  // over tens of thousands of millisecond timestamps). CAST(... AS TEXT) in
  // SQL sidesteps DatabaseSync's RangeError on oversized INTEGERs; parse the
  // text as BigInt so drift comparison stays exact.
  ...
  const sum = (key: string): bigint => bigint(isRecord(counts) ? counts[key] : undefined);
  return {
    dataVersion: number(isRecord(dataVersionRow) ? dataVersionRow.data_version : undefined),
    trajectoryBytes: sum("trajectory_bytes"),
    trajectoryRows: count("trajectory_rows"),
    transcriptBytes: sum("transcript_bytes"),
    transcriptCreatedAt: sum("transcript_created_at"),
